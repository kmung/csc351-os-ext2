...
int main() {
    ...
    fs myFs;
    struct stat fileStat; //for stat functions
    ...
    //Testing my_fstat function
    //use the file descriptor from the open function
    int fstatResult = myFs.my_fstat(openFd, fileStat);
    if (statResult == -1) {
        cerr << "Error getting file status." << endl;
    }
}